...
static IoT_Error_t executeSystemCmd(const char *cmd)
{
										  ...
										  IoT_Publish_Message_Params *params, void *pData)
{
	...
	char cmdToBeExecuted[MAX_SIZE_OF_CMD + 1];
	IoT_Error_t rc;
	...
	if ((tokExecution = findToken("execution", params->payload, jsonTokenStruct)) == NULL)
	{
		IOT_INFO("execution property not found, nothing to do");
	}
	else if ((tokJobId = findToken("jobId", params->payload, tokExecution)) == NULL)
	{
		IOT_INFO("jobId property not found, nothing to do");
	}
	else if ((rc = parseStringValue(jobId, MAX_SIZE_OF_JOB_ID + 1, params->payload, tokJobId)) != SUCCESS)
	{
		IOT_ERROR("parseStringValue returned error : %d ", rc);
	}
	else
	{
		if ( ( tokJobDocument = findToken("jobDocument", params->payload, tokExecution) ) == NULL )
		{
			IOT_INFO("tokJobDocument property not found, nothing to do");
			iot_update_job_status(pClient, jobId, JOB_EXECUTION_SUCCEEDED, "{\"reason\":\"Nop\"}");
		}
		else if ( ( tok = findToken("cmdToBeExecuted", params->payload, tokJobDocument) ) == NULL )
		{
			IOT_INFO("cmdToBeExecuted property not found, nothing to do");
			iot_update_job_status(pClient, jobId, JOB_EXECUTION_SUCCEEDED, "{\"reason\":\"Nop\"}");
		}
		else if ( (tok->end - tok->start) > MAX_SIZE_OF_CMD )
		{
			IOT_ERROR("cmdToBeExecuted property execeeds max size, skip this command");
			iot_update_job_status(pClient, jobId, JOB_EXECUTION_FAILED, "{\"reason\":\"CmdFormatError\"}");
		}
		else if ( ( rc = parseStringValue(cmdToBeExecuted, MAX_SIZE_OF_CMD + 1, params->payload, tok) ) != SUCCESS )
		{
			IOT_ERROR("parseStringValue returned error : %d ", rc);
			iot_update_job_status(pClient, jobId, JOB_EXECUTION_FAILED, "{\"reason\":\"CmdFormatError\"}");
		}
		else if ( ( rc = executeSystemCmd(cmdToBeExecuted) ) != SUCCESS )
		{
			IOT_ERROR("executeSystemCmd returned error : %d ", rc);
			iot_update_job_status(pClient, jobId, JOB_EXECUTION_FAILED, "{\"reason\":\"CmdExeError\"}");
		}
		else
		{
			IOT_INFO("executeSystemCmd returned success");
			iot_update_job_status(pClient, jobId, JOB_EXECUTION_SUCCEEDED, "{\"reason\":\"CmdExeSuccess\"}");
		}
	}
}
